...
class ResidualBlock(nn.Module):
    ...
    @nn.compact
    def __call__(
        self, x: jnp.ndarray, *, norm_kind: str | None = "layernorm", train: bool
    ) -> jnp.ndarray:
        norm = make_norm(norm_kind)
        h = x if norm is None else norm(x)
    ...
    def __call__(
        ...
        if self.cfg.cat_cardinalities:
            for i, card in enumerate(self.cfg.cat_cardinalities):
                emb = nn.Embed(
                    num_embeddings=int(card), features=int(self.cfg.embed_dim)
                )
        ...
        for _ in range(int(self.cfg.blocks)):
            x = ResidualBlock(width=self.cfg.width, dropout=self.cfg.dropout)(
                x, norm_kind=self.cfg.norm_kind, train=train
            )

        norm = make_norm(self.cfg.norm_kind)
        if norm is not None:
            x = norm(x)
        x = nn.Dense(1)(x)
...
def init_model(
    rng: jax.Array, cfg: ModelConfig, cat_cols: int
) -> tuple[TabularEmbedMLP, dict]:
    ...
    variables = model.init(
        {"params": rng, "dropout": rng}, dummy_cat, dummy_num, train=True
    )
